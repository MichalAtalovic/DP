        ...
        while tries < self._max_retries:
            try:
                ...
                if resp.status_code == 200 and not has_captcha:
                    ...
                    print('Got a captcha request.')
                    os.system('cls' if os.name == 'nt' else 'clear')
                    
                    operaConfig = []
                    with codecs.open(os.path.dirname(__file__) + "\\..\\..\\..\\webconfig.json", 'r', 'utf-8') as json_file:
                        operaConfig = json.load(json_file)
                    
                    options = webdriver.ChromeOptions() 
                    options.add_argument("--user-data-dir=" + operaConfig['Scraper']['OperaProfileDir'])
                    browser = webdriver.Opera(executable_path = os.path.dirname(__file__) + "\\..\\..\\opera\\operadriver.exe", options=options)
                    browser.minimize_window()
                    browser.get(pagerequest)

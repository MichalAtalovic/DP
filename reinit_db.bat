...
psql -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE pid <> pg_backend_pid() AND datname = 'pubciter'" "user=postgres dbname=postgres password=postgres"
REM drop database
...
REM create model
psql -c "\i scripts/create_tables.sql" "user=postgres dbname=pubciter password=postgres"
PAUSE
